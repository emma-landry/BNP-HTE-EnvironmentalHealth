...
strata_true_s1 <- lapply(1:samples, function(s) {
  ...
  cut(S, breaks = quantile(S, probs = c(0, 1/3, 2/3, 1)), labels = c("Low","Mid","High"), include.lowest = TRUE)
})
...
strata_BPCF_s1 <- lapply(1:samples, function(s) {
  ...
  cut(S_hat, breaks = quantile(S_hat, probs = c(0, 1/3, 2/3, 1)), labels = c("Low","Mid","High"),include.lowest = TRUE)
})
...
strata_CASBAH_s1 <- lapply(1:samples, function(s) {
  relabel_CASBAH_strata(deltaP = CASBAH_scenario1[[s]]$post_P_1_imp - CASBAH_scenario1[[s]]$post_P_0_imp,
                        strata_raw = CASBAH_scenario1[[s]]$S_strata_cluster)
})

strata_CASBAH_s2 <- lapply(1:samples, function(s) {
  relabel_CASBAH_strata(deltaP = CASBAH_scenario2[[s]]$post_P_1_imp - CASBAH_scenario2[[s]]$post_P_0_imp,
...
make_EP_strata <- function(pt_hat, strata, method, scenario) {
  do.call(rbind, lapply(1:samples, function(s) {
    data.frame(value = tapply(pt_hat[, s], strata[[s]], mean),
...
# Bias ----
# Bias: marginal P
bias_P_df <- rbind(data.frame(value = sapply(1:samples, function(s) compute_ATE(BPCF_pt_1[, s], simulate_pt_1[, s])$bias),
                              Method = "BPCF", Scenario = "Scenario 1"),
                   data.frame(value = sapply(1:samples, function(s) compute_ATE(CASBAH_pt_1[, s], simulate_pt_1[, s])$bias),
                              Method = "CASBAH", Scenario = "Scenario 1"),
                   data.frame(value = sapply(1:samples, function(s) compute_ATE(BPCF_pt_2[, s], simulate_pt_2[, s])$bias),
                              Method = "BPCF", Scenario = "Scenario 2"),
                   data.frame(value = sapply(1:samples, function(s) compute_ATE(CASBAH_pt_2[, s], simulate_pt_2[, s])$bias),
                              Method = "CASBAH", Scenario = "Scenario 2"))

# Bias: marginal Y
bias_Y_df <- rbind(data.frame(value = sapply(1:samples, function(s) compute_ATE(BPCF_tau_1[, s], simulated_tau_1[, s])$bias),
                              Method = "BPCF", Scenario = "Scenario 1"),
                   data.frame(value = sapply(1:samples, function(s) compute_ATE(CASBAH_tau_1[, s], simulated_tau_1[, s])$bias),
                              Method = "CASBAH", Scenario = "Scenario 1"),
                   data.frame(value = sapply(1:samples, function(s) compute_ATE(BPCF_tau_2[, s], simulated_tau_2[, s])$bias),
                              Method = "BPCF", Scenario = "Scenario 2"),
                   data.frame(value = sapply(1:samples, function(s) compute_ATE(CASBAH_tau_2[, s], simulated_tau_2[, s])$bias),
                              Method = "CASBAH", Scenario = "Scenario 2"))
...
make_ATE_bias_strata <- function(tau_hat, tau_true, strata, method, scenario) {
  do.call(rbind, lapply(1:samples, function(s) {
    ...
    data.frame(value = ate_hat - ate_true,
               Group = names(ate_hat),
               Method = method,
               Scenario = scenario)
  }))
...
bias_Y_strata_df <- rbind(make_ATE_bias_strata(BPCF_tau_1,   simulated_tau_1, strata_BPCF_s1,   "BPCF",   "Scenario 1"),
...
EP_strata_df <- rbind(make_EP_strata(BPCF_pt_1,   strata_BPCF_s1,   "BPCF",   "Scenario 1"),
...
force_LMH <- function(df) {
  df$Group <- factor(df$Group,
                     ...
                     labels = c("Associative\nNegative", "Dissociative", "Associative\nPositive"))
  df
}
...
EP_true_s1 <- do.call(rbind, lapply(1:samples, function(s) {
  data.frame(value = tapply(simulate_pt_1[, s], strata_true_s1[[s]], mean),
...
p1_s1 <- ggplot(subset(EP_strata_df, Scenario == "Scenario 1"), aes(Group, value, fill = Method, color = Method)) +
  geom_boxplot(position = position_dodge(0.7), alpha = 0.35) +
  geom_segment(data = EP_true_s1_mean, aes(x = as.numeric(Group) - 0.45, xend = as.numeric(Group) + 0.45, y = value,yend = value),
...
# Column 2
p2_s1 <- ggplot(subset(bias_P_df, Scenario == "Scenario 1"), aes(Method, value, fill = Method, color = Method)) +
...
# Column 3
p3_s1 <- ggplot(subset(bias_Y_strata_df, Scenario == "Scenario 1"), aes(Group, value, fill = Method, color = Method)) +
  geom_boxplot(position = position_dodge(0.7), alpha = 0.35) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  scale_y_continuous(limits = sym_limits) +
  labs(x = NULL, y = "Bias") +
...
# Column 4
p4_s1 <- ggplot(subset(bias_Y_df, Scenario == "Scenario 1"), aes(Method, value, fill = Method, color = Method)) +
  geom_boxplot(alpha = 0.35) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  scale_y_continuous(limits = sym_limits) +
  labs(x = NULL, y = "Bias") +
...
EP_true_s2 <- do.call(rbind, lapply(1:samples, function(s) {
  data.frame(value = tapply(simulate_pt_2[, s], strata_true_s2[[s]], mean),
...
p1_s2 <- ggplot(subset(EP_strata_df, Scenario == "Scenario 2"), aes(Group, value, fill = Method, color = Method)) +
  geom_boxplot(position = position_dodge(0.7), alpha = 0.35) +
  geom_segment(data = EP_true_s2_mean, aes(x = as.numeric(Group) - 0.45, xend = as.numeric(Group) + 0.45, y = value, yend = value),
               inherit.aes = FALSE, linetype = "dashed", linewidth = 0.5) +
  labs(x = NULL, y = "Value") +
  facet_wrap(~"E[P(1) - P(0) | strata]") +
  scale_fill_manual(values = paper_colors) +
...
# Column 3
p3_s2 <- ggplot(subset(bias_Y_strata_df, Scenario == "Scenario 2"), aes(Group, value, fill = Method, color = Method)) +
  geom_boxplot(position = position_dodge(0.7), alpha = 0.35) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  scale_y_continuous(limits = sym_limits) +
  labs(x = NULL, y = "Bias") +
...
# Final figure ------
scenario_title <- function(label) {ggplot() +
...
fig_final <- scenario_title("Scenario 1") / row_s1 / scenario_title("Scenario 2") / row_s2 +
